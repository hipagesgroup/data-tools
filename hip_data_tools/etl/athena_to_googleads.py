...
class AthenaToAdWordsOfflineConversion(AthenaToDataFrame):
    ...
    def _get_adwords_util(self):
        if self._adwords is None:
            self._adwords = GoogleAdsOfflineConversionUtil(
                GoogleAdsConnectionManager(
    ...
    def _verify_data_before_upsert(self, data: List[dict]) -> (List[dict], List[dict]):
        ...
        if len(issues) > 0:
            LOG.warning(
                "Issues found in verification, number of issues: %i", len(issues)
            )
    ...
    def _sanitise_data(self, dat):
        try:
            ...
            if current_state == EtlStates.Ready:
                ...
                return (
                    None,
                    _get_structured_issue(
                        f"Current state is {current_state} " "state", dat
                    ),
                )
        except ValidationError as e:
            LOG.warning(
